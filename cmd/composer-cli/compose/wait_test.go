...
func TestCmdComposeWaitUnknownJSON(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Contains(t, string(stdout), "Waiting ")
...
func TestCmdComposeWaitWaiting(t *testing.T) {
	...
	defer out.Close()
	require.NotNil(t, err)
	assert.ErrorContains(t, err, "timeout after 2s")
	require.NotNil(t, out.Stdout)
	require.NotNil(t, out.Stderr)
	require.NotNil(t, cmd)
	assert.Equal(t, cmd, waitCmd)
	...
	stderr, err := io.ReadAll(out.Stderr)
	assert.Nil(t, err)
	assert.Equal(t, []byte("ERROR: timeout after 2s\n"), stderr)
	assert.Equal(t, "GET", mc.Req.Method)
}
...
func TestCmdComposeWaitFailed(t *testing.T) {
	...
	stdout, err := io.ReadAll(out.Stdout)
	assert.Nil(t, err)
	assert.Contains(t, string(stdout), "ddcf50e5-1ffa-4de6-95ed-42749ac1f389 FAILED\n")
	stderr, err := io.ReadAll(out.Stderr)
	assert.Nil(t, err)
