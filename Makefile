...
BUILDFLAGS ?= -ldflags="-X github.com/weldr/weldr-client/cmd/composer-cli/root.Version=${VERSION}"
GOBUILDFLAGS ?= 
...
test:
	go test ${BUILDFLAGS} ${GOBUILDFLAGS} -v -race -covermode=atomic -coverprofile=coverage.txt -coverpkg=./... ./...
